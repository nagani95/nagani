    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 mx-auto w-full max-w-md px-2 pb-[calc(0.38rem+env(safe-area-inset-bottom))]">
      <div className="relative h-[8.25rem] overflow-visible">
        <img
          ...
          alt=""
          aria-hidden="true"
          className="pointer-events-none absolute left-1/2 bottom-[0.12rem] h-[6.15rem] w-[104%] max-w-none -translate-x-1/2 select-none object-fill drop-shadow-[0_-16px_32px_rgba(0,0,0,0.74)]"
          draggable={false}
        />

        <div className="absolute left-1/2 bottom-[0.95rem] z-20 grid w-[104%] -translate-x-1/2 grid-cols-3 items-center px-8">
          {navItems.map((item) => {
